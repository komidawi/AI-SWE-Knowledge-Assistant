...
1. Read `kb/planning/<year>/<year>.md` and the current month file for `weekly_hours`, focus and targets.
...
4. Pull candidates from `kb/resources/` with `status: in-progress` (finish what is started before
   starting more) and `kb/ideas/` with `status: accepted`.
5. Ask for this week's real available hours if `capacity_hours` is unset. Do not assume.
6. Write the week file from `kb/templates/week.md`. Rules:
    ...
    - Tie-break on the stack. Two items advancing goals equally: the one whose topics are in
      `kb/taxonomy/stack.yml` wins. An item on neither axis is not committed at all.
