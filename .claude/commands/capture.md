...
2. If a URL was given, fetch it to get the real title, author and scope. Do not guess metadata that
   the page states.
3. Check `kb/ideas/`, `kb/resources/` and `kb/goals/` for an existing entry on the same thing. If one exists,
   update it rather than creating a near-duplicate, and say that is what you did.
4. Copy the matching file from `kb/templates/` and fill every field. For a resource, `scale:` follows
...
5. Map topics to ids in `kb/taxonomy/topics.yml`. If a needed topic is missing, add it to the taxonomy
...
7. Set `priority:` from the two axes: serves an active goal, or is on-stack per `kb/taxonomy/stack.yml`.
