...
| Script              | Does                                                                                                                                  |
|---------------------|---------------------------------------------------------------------------------------------------------------------------------------|
| `dashboard.mjs`     | Writes `README.md` (repo front page) and its light/dark SVG charts in `system/assets/dashboard/`: needs attention, goals, this week, in flight, areas, pick by time. Never hand-edit the output. |
...
| `toggl.mjs [week|from to] [--json]` | Actual hours from Toggl Track (env `TOGGL_API_TOKEN`), grouped by entry description, matched to kb ids by id/title. Read-only. Unmatched rows print `?`. Default: current ISO week. |
